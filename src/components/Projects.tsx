...
const Projects = () => {
  ...
  return (
    <section className="bg-yellow px-3 py-5 dark:bg-[#494949] md:px-0 md:py-10">
      ...
      <div className="grid w-full grid-cols-2 place-content-center place-items-center gap-4">
        {repo?.map((item: any) => (
          <a
            className=" w-full max-w-md cursor-pointer rounded-md bg-[#fff] shadow-lg transition-all hover:scale-110 dark:bg-[#827f7f] dark:text-white"
            href={item.html_url}
            key={item.id}
            target="_blank"
          >
            <div className="flex h-full w-full flex-col justify-between gap-5 p-4">
              <div className="flex items-center">
                <div className="text-base-content flex items-center gap-2 text-lg tracking-wide opacity-60">
